...
sed -i 's/\r$//' install.sh skill/scripts/*.sh   # if CRLF checkout breaks bash on WSL/Git Bash
...
- Mode argument 2 of `send-webhook.sh` is mandatory in practice: omitting it silently defaults to `batch` and the email only leaves with the morning cron.
